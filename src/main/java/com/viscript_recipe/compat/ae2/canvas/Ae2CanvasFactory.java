...
/** Positions AE2 slots in logical JEI pixels; item icons remain centered in their 18-pixel cells. */
public final class Ae2CanvasFactory {
    private Ae2CanvasFactory() {}
    ...
    public static Label label(Component text, int x, int y, int width) {
        var label = RecipeEditorUi.label(text).textStyle(style -> style.fontSize(7));
        return (Label) at(label, x, y, width, 12);
    }
}
